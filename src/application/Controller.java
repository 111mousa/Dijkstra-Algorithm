...
public class Controller implements Initializable {
	...
	void run(ActionEvent event) throws IOException {
		...
		if(file.length()>0&&src1.compareTo(source.getValue())==0&&trgt1.compareTo(target.getValue())==0) {
			getData();
		...
		else if (source.getValue() != "none" && target.getValue() != "none") {
			src1 = source.getValue();
			trgt1 = target.getValue();
			writeToTheFile(addAdjacents1(source.getValue(),target.getValue()));
			getDistances();
...
//			distanceView.setText(df.format(t.getDistance()));
	...
	private void addAdjacents2() {
		for(City city: cities){
			city.getAdjacent().clear();
		}
		for (int i = 0; i < cities.size(); i++) {
	 ...
	 */
	private void getDistances2() throws FileNotFoundException {
		File file = new File("distance");
 ...
 * Build the hash table by applying the dijkstra algorithm
	*/
	...
	private void writeToTheFile(String str) throws IOException {
		FileWriter file = new FileWriter("distance");
		file.write(str);
		file.close();
	}
	/*
	...
	private void getCities() throws FileNotFoundException {
		...
		while (input.hasNextLine()) {
			...
			double distance1 =  (741 - (((Double.parseDouble(spStr[1])+90)/180)*741))-50;
			double distance2 =  (((Double.parseDouble(spStr[2])+180)/360)*1200)-100;
			cities.add(new City(distance2, distance1, spStr[0], new Circle()));
		}
		input.close();
	 ...
	 */
	private double getDistanceBetweenTwoCities(City city1,City city2) {
		return Math.ceil(Math.abs(Math.sqrt(((Math.pow(city2.getCoordinateX()-city1.getCoordinateX(), 2)+Math.pow(city2.getCoordinateY()-city1.getCoordinateY(), 2))))));
	}
	private int getNumberOfAdjacent() {
		Random rand = new Random();
		int r = rand.nextInt(4);
		return r==0?r+1:r;
	}
	private String addAdjacents1(String source,String target) {
		int [] set = new int[cities.size()];
		Random r = new Random();
		int adj = r.nextInt(cities.size());
		String str = "";
		int src = cities.indexOf(getCity(source));
		int targ = cities.indexOf(getCity(target)),numOfAdj=0, noa = getNumberOfAdjacent();
		Queue<Integer>q = new LinkedList<Integer>();
		while(adj!=targ) {
			if(set[adj]==1||src==adj) {
			...
			if(numOfAdj==noa) {
				numOfAdj=0;
				noa = getNumberOfAdjacent();
				set[src]=1;
				src = q.poll();
			}
			City c = cities.get(adj);
			double distance = getDistanceBetweenTwoCities(cities.get(src), c);
			Adjacent a = new Adjacent(c, distance);
			cities.get(src).getAdjacent().add(a);
			str+=cities.get(src).getName()+" "+a.getCity().getName()+" "+a.getDistance()+"\n";
			numOfAdj++;
			q.add(adj);
			adj = r.nextInt(cities.size());
		}
		City c = cities.get(adj);
		double distance = getDistanceBetweenTwoCities(cities.get(src), c);
		cities.get(src).getAdjacent().add(new Adjacent(c,distance));
		str+=cities.get(src).getName()+" "+c.getName()+" "+distance+"\n";
		return str;
}
